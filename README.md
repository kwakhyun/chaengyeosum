...
- 초대 코드 검증과 익명 참여 세션
- 참가자별 해시 처리된 접근 토큰
- 앱인토스 `getAnonymousKey` 기반 사용자별 내 모임 자동 복구
- 익명 식별키 원문을 저장하지 않는 SHA-256 서버 매핑
...
- `getAnonymousKey`: 로그인 없이 앱별 사용자를 식별하고 내 모임 복구
...
- 토스 익명 식별키도 원문 대신 SHA-256 해시만 저장합니다.
- 일반 브라우저와 미지원 토스 앱은 기존 로컬 참여 토큰으로 동작합니다.
